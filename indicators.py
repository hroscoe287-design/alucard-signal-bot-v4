...
def cci(df,n=14):
 tp=(df.high+df.low+df.close)/3; ma=tp.rolling(n).mean(); md=tp.rolling(n).apply(lambda x:np.mean(np.abs(x-x.mean())),raw=True); return (tp-ma)/(0.015*md.replace(0,np.nan))
def macd(s):
...
def fractal(df,span=2): return df.high.rolling(2*span+1,center=True).max().eq(df.high),df.low.rolling(2*span+1,center=True).min().eq(df.low)
def calculate(candles):
 if len(candles)<35:return {"ready":False,"reason":"Need at least 35 candles","values":{}}
 df=pd.DataFrame(candles); close=df.close.astype(float); e9,e20,e50=ema(close,9),ema(close,20),ema(close,50); m,ms,mh=macd(close); ps=psar(df); jaw,teeth,lips=alligator(df); fu,fd=fractal(df,2)
 last=lambda s:float(s.iloc[-1]) if pd.notna(s.iloc[-1]) else None
 return {"ready":True,"values":{"price":float(close.iloc[-1]),"ema9":last(e9),"ema20":last(e20),"ema50":last(e50),"macd":last(m),"macd_signal":last(ms),"macd_hist":last(mh),"rsi":last(rsi(close)),"cci":last(cci(df)),"atr":last(atr(df)),"psar":last(ps),"alligator_jaw":last(jaw),"alligator_teeth":last(teeth),"alligator_lips":last(lips),"fractal_up":bool(fu.iloc[-3]),"fractal_down":bool(fd.iloc[-3])}}
